      ...
      <div class="myorder_main">
          ...
          <div class="myorder_content">
            <div class="myorder_mainTop">
              <el-row>
                <div class="all_ordersBox">
                    <div class="search_title">
                        <el-row class="search" style="padding-top: 2%">
                            <div>
                                <el-select v-model="value" size="small" placeholder="请选择搜索方式">
                    ...
                    <div class="product_main"  v-show="boxShow" v-for="myorder_product in dataBox" :key="myorder_product.id" >
                            ...
                            <router-link :to="'/order/detail?orderId='+myorder_product.id" style="color:#000">
                              ...
                              <el-col :sm="{ span:11}" class="operation">
                                ...
                                <el-col v-show="status(myorder_product.status, 'YTK')">
                                  <router-link :to="'/return/request?orderId='+myorder_product.id">
                                    <button type="button" style="margin-top: 8%;">退货详情</button>
                                  </router-link>
                                </el-col>
                              </el-col>
                            </el-col>
                    ...
                    <div class="prompt_text" v-show="!boxShow">
                        <p>没有搜索到订单哦~</p>
                    </div>
                </div>
              </el-row>
...
export default {
  ...
  mounted() {
      ...
      let that=this;
      this.value=that.$route.query.type;
      that.search_text=that.$route.query.searchInfo;
      that.search_type=that.$route.query.type;
      this.axios.get('https://grouponapi.top/api/userorder/record/serch?type='+that.search_type+'&serch_info='+that.search_text+'&page='+that.page +'&include=orderDetail,orderDetail.product')
       .then(res => {
  ...
  methods: {
    ...
    searchBtn(){
      ...
      if(this.searchVal == ''){
        ...
        that.search_text=that.searchVal;
        that.axios.get('https://grouponapi.top/api/userorder/record/serch?type='+that.value+'&serch_info='+that.search_text+'&page='+that.page +'&include=orderDetail,orderDetail.product')
        .then(res => {
            that.dataBox=res.data.data
            if(that.dataBox.length == 0){
                that.boxShow=false
            }else{
                that.boxShow=true
            }
